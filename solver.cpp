...
ComplexVariable::ComplexVariable(const complex<double> a, const complex<double> b, const complex<double> c)
{
    this->_a;
    this->_b;
    this->_c;
...
ComplexVariable solver::operator^(const ComplexVariable& x,const int k) // (a+bi)^k
{
    ...
    else if(k == 1) 
    {
        ...
        complex<double> new_a = x._a + x._b * x._b;
        complex<double> new_b = 0;
        complex<double> new_c = x._c;
        return ComplexVariable(new_a,new_b,new_c);
    } 
    else 
        throw runtime_error("invalid"); 
...
complex<double> solver::solve(const ComplexVariable& x)
{
    if(x._a == complex<double>(0,0))
    {
        ...
        complex<double> ans = ((complex<double> (-1) * x.b ) + sqrt((x.b * x.b) - (complex<double> (4, 0) *x.a*x.c))/(complex<double>(2, 0) * x.a);
        return ans;
} 
